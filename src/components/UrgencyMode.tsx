...
function UrgencyCard({ task }: Props) {
  ...
  return (
    <div className="w-full max-w-sm space-y-6 animate-welcome">
      ...
      <div className="flex items-center gap-2 justify-center">
        ...
        <span className="text-xs font-semibold text-rose-300/80 uppercase tracking-widest">
          Mode Urgence
        </span>
      </div>
        ...
        <div className="bg-white/6 border border-white/10 rounded-2xl p-4 space-y-2 text-left">
          {task.microSteps.map((step) => (
            <div key={step.id} className="flex items-center gap-2">
              <div className={`w-3 h-3 rounded-full border flex-shrink-0 ${step.done
                ? 'bg-emerald-400 border-emerald-400'
                : 'border-white/30'}`}
              />
              <span className={`text-sm ${step.done ? 'text-white/30 line-through' : 'text-white/70'}`}>
                {step.label}
              </span>
            </div>
          ))}
      ...
      {!started && (
        <div className="space-y-3">
          ...
          <button
            onClick={handleStart}
            className="w-full flex items-center justify-center gap-3 py-4 rounded-2xl bg-rose-600 hover:bg-rose-500 active:scale-95 text-white font-semibold text-base transition-all shadow-2xl shadow-rose-900/50"
          >
            <Rocket size={18} />
            Je démarre
          </button>
      ...
      {started && (
        <button
          onClick={handleDone}
          className="w-full flex items-center justify-center gap-3 py-4 rounded-2xl bg-emerald-600/20 border border-emerald-500/30 hover:bg-emerald-600/35 active:scale-95 text-emerald-300 font-semibold text-base transition-all"
        >
          <Check size={18} />
          C'est fait
        </button>
...
export default function UrgencyMode() {
  const { tasks, setUrgencyMode } = useStore()
  const urgentTask = tasks.find((t) => !t.done && t.context === 'urgent')
    ?? tasks.find((t) => !t.done)
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-[#0d0d1a] overflow-hidden">
      ...
      />
      <div
        className="pointer-events-none fixed bottom-[-20%] right-[5%] w-[400px] h-[400px] rounded-full animate-pulse-glow"
        style={{ background: 'radial-gradient(circle, #9f1239, transparent)', animationDelay: '1.5s' }}
      />
