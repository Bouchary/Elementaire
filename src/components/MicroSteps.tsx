...
export default function MicroSteps({ task }: Props) {
  ...
  return (
    <div className="mt-3 space-y-2">
      {task.microSteps.map((step) => (
        <div key={step.id} className="flex items-center gap-2 group">
          ...
          >
            <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all
              ${step.done
                ...
                : 'border-white/35 group-hover:border-white/65'}`}>
              {step.done && <Check size={9} className="text-black" strokeWidth={3} />}
            </div>
            <span className={`text-sm transition-all ${step.done
          ...
          <button
            ...
            className="opacity-0 group-hover:opacity-100 p-1 rounded-lg text-white/25 hover:text-rose-400 hover:bg-rose-400/10 transition-all flex-shrink-0"
          >
            <X size={12} />
          </button>
        </div>
      ...
      <div className="flex items-center gap-2 mt-3 pt-2 border-t border-white/8">
        ...
        <button
          onClick={handleAdd}
          className="p-1 rounded-lg bg-white/10 hover:bg-white/20 text-white/60 hover:text-white transition-all"
        >
          <Plus size={14} />
        </button>
      </div>
