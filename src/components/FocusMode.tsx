...
export default function FocusMode({ task, onClose, onTaskChange }: Props) {
      ...
      <div className="flex-1 flex items-center justify-center px-6">
        <div className="w-full max-w-lg space-y-6">
          ...
          <div className="bg-white/8 border border-white/15 rounded-2xl p-5">
            <MicroSteps
              task={task}
              onStepChange={() => onTaskChange?.(useStore.getState().tasks.find((t) => t.id === task.id)!)}
            />
          </div>
          <CandleTimer taskId={task.id} />
          <button
            onClick={handleDone}
            className="flex items-center justify-center gap-2 w-full py-4 rounded-2xl bg-emerald-500/20 border border-emerald-400/40 text-emerald-300 hover:bg-emerald-500/30 transition-all text-base font-semibold"
          >
            <Check size={18} />
